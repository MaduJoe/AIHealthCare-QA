...
test('페이지가 로드되고 올바른 타이틀을 가진다', async ({ page }) => {
  ...
  await expect(page).toHaveTitle(/Lunit Insight CXR/i); // Use regex for flexibility

  // Optional: Check for a key element confirming load, like the main header
  await expect(page.locator('h1')).toContainText('Lunit Insight CXR'); // Adjust selector/text if needed
}); 